...
def main():
    ...
    page = [
        ...
        "function resetQuiz(){quizDiv.innerHTML=''; quizQuestions=[]; currentQuestionIndex=0; quizBtn.disabled=true; quizStatus.textContent='Select the Forces unit to use its pre-generated questions.';}",
        ...
        "function renderQuestion(){const item=quizQuestions[currentQuestionIndex]; quizDiv.innerHTML=''; const heading=document.createElement('h3'); heading.textContent='Forces question'; quizDiv.appendChild(heading); const article=document.createElement('article'); article.className='quiz-question'; const question=document.createElement('p'); question.textContent=item.question; article.appendChild(question); const options=document.createElement('ol'); options.className='quiz-options'; options.type='A'; item.options.forEach(option=>{const li=document.createElement('li');li.textContent=option;options.appendChild(li);}); article.appendChild(options); const answer=document.createElement('p'); answer.className='correct-answer'; answer.hidden=true; answer.textContent=`Correct answer: ${String.fromCharCode(65+item.correctAnswerIndex)}. ${item.options[item.correctAnswerIndex]}`; article.appendChild(answer); quizDiv.appendChild(article); const controls=document.createElement('div'); controls.className='quiz-navigation'; const previous=document.createElement('button'); previous.type='button';previous.textContent='←';previous.setAttribute('aria-label','Previous question');previous.addEventListener('click',()=>{currentQuestionIndex=(currentQuestionIndex-1+quizQuestions.length)%quizQuestions.length;renderQuestion();}); const showAnswer=document.createElement('button'); showAnswer.type='button';showAnswer.textContent='Show answer';showAnswer.addEventListener('click',()=>{answer.hidden=false;showAnswer.remove();}); const next=document.createElement('button'); next.type='button';next.textContent='→';next.setAttribute('aria-label','Next question');next.addEventListener('click',()=>{currentQuestionIndex=(currentQuestionIndex+1)%quizQuestions.length;renderQuestion();}); controls.append(previous,showAnswer,next);quizDiv.appendChild(controls);}",
        "async function generateQuiz(){if(sel.value!==QUIZ_UNIT)return; quizBtn.disabled=true; quizStatus.textContent='Loading questions...'; try{await loadQuizQuestions(); currentQuestionIndex=Math.floor(Math.random()*quizQuestions.length); renderQuestion(); quizStatus.textContent='';}catch(error){quizStatus.textContent=error.message;}finally{if(sel.value===QUIZ_UNIT)quizBtn.disabled=false;}}",
