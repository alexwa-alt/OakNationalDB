...
def main():
    ...
    page = [
        ...
        "function resetQuiz(){quizDiv.innerHTML=''; quizQuestions=[]; selectedQuestionIndexes=[]; quizBtn.disabled=true; quizStatus.textContent='Select a unit to use its pre-generated questions.';}",
        "function showPoints(unit){pointsDiv.innerHTML=''; resetQuiz(); if(!unit) return; const items=UNITS[unit]||[]; if(items.length===0){pointsDiv.textContent='No key learning points found.'; return;} const frag=document.createDocumentFragment(); items.forEach(p=>{const div=document.createElement('div');div.textContent=p;frag.appendChild(div)}); pointsDiv.appendChild(frag); copyBtn.textContent='Copy all'; quizBtn.disabled=false; quizStatus.textContent='Generate a quiz from this unit’s cached questions.';}",
