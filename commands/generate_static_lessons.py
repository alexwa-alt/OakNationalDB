...
def main():
    ...
    page = [
        ...
        "<style>body{font-family:system-ui,-apple-system,'Segoe UI',Roboto,Arial;padding:1rem;max-width:70rem}nav{margin-bottom:1rem}select,button{font-size:1rem;padding:.35rem;margin-bottom:1rem}#points{white-space:pre-wrap}.quiz-question{margin:1.25rem 0;padding:1rem;border:1px solid #d0d7de;border-radius:.4rem}.quiz-options{padding-left:1.5rem}.quiz-options li{margin:.4rem 0}.correct-answer{font-weight:600;color:#116329}.quiz-status{margin-top:.5rem;color:#57606a}.quiz-navigation{display:flex;gap:.5rem;align-items:center}.quiz-navigation button{margin:0}</style>",
        "</head><body>",
        "<nav><a href=\"index.html\">Home</a> | <a href=\"new-page.html\">New page</a></nav>",
        "<h1>KS3 Science — key learning points by unit</h1>",
        "<p>Select a unit to generate a quiz from its key learning points.</p>",
        ...
        "<h2 id=\"quiz-heading\">Unit quiz</h2>",
        "<button id=\"quiz-btn\" type=\"button\" disabled>Generate quiz</button>",
        "<p id=\"quiz-status\" class=\"quiz-status\">Select the Forces unit to use its pre-generated questions.</p>",
        ...
        "const QUIZ_UNIT = '1ForcesPhysics';",
        "const QUIZ_INDEX_URL = 'quizzes/forces/index.json';",
        ...
        "function resetQuiz(){quizDiv.innerHTML=''; quizQuestions=[]; selectedQuestionIndexes=[]; quizBtn.disabled=true; quizStatus.textContent='Select the Forces unit to use its pre-generated questions.';}",
        "function showPoints(unit){pointsDiv.innerHTML=''; resetQuiz(); if(!unit) return; const items=UNITS[unit]||[]; if(items.length===0){pointsDiv.textContent='No key learning points found.'; return;} const frag=document.createDocumentFragment(); items.forEach(p=>{const div=document.createElement('div');div.textContent=p;frag.appendChild(div)}); pointsDiv.appendChild(frag); copyBtn.textContent='Copy all'; if(unit===QUIZ_UNIT){quizBtn.disabled=false; quizStatus.textContent='Pre-generated questions are available for this unit.';}}",
        ...
        "async function loadQuizQuestions(){if(quizQuestions.length)return; const response=await fetch(QUIZ_INDEX_URL,{cache:'no-store'}); if(!response.ok)throw new Error('Quiz cache is not published yet.'); const data=await response.json(); if(!Array.isArray(data.quizzes)||!data.quizzes.every(name=>typeof name==='string'))throw new Error('Quiz cache index is invalid.'); const quizzes=await Promise.all(data.quizzes.map(async filename=>{const quizResponse=await fetch(`quizzes/forces/${filename}`,{cache:'no-store'});if(!quizResponse.ok)throw new Error('A cached quiz could not be loaded.');return quizResponse.json();})); quizQuestions=quizzes.flatMap(quiz=>Array.isArray(quiz.questions)?quiz.questions:[]); if(!quizQuestions.length)throw new Error('No cached questions are available.'); quizQuestions.sort(()=>Math.random()-.5);}",
        "function swapQuestion(slot,direction){const unavailable=new Set(selectedQuestionIndexes);unavailable.delete(selectedQuestionIndexes[slot]);const current=selectedQuestionIndexes[slot];for(let offset=1;offset<quizQuestions.length;offset++){const candidate=(current+direction*offset+quizQuestions.length)%quizQuestions.length;if(!unavailable.has(candidate)){selectedQuestionIndexes[slot]=candidate;renderQuestions();return;}}}",
        "function printQuiz(){const questions=selectedQuestionIndexes.map(index=>quizQuestions[index]); if(questions.length!==QUESTION_COUNT){alert('Generate a complete quiz before printing.');return;} const printWindow=window.open('','_blank');if(!printWindow){alert('Please allow pop-ups to print the quiz.');return;} const title=escapeHtml(prettifyLabel(sel.value));const pages=[questions.slice(0,10),questions.slice(10,20)].map((page,pageIndex)=>`<section class=\"quiz-page\"><h1>${title} quiz</h1><div class=\"questions\">${page.map((item,index)=>`<article class=\"question\"><p><strong>${pageIndex*10+index+1}.</strong> ${escapeHtml(item.question)}</p><ol type=\"A\">${item.options.map(option=>`<li>${escapeHtml(option)}</li>`).join('')}</ol></article>`).join('')}</div></section>`).join('');printWindow.onload=()=>{printWindow.focus();printWindow.print();};printWindow.document.write(`<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\"><title>${title} quiz</title><style>@page{size:A4;margin:12mm}body{font-family:Arial,sans-serif;color:#000;margin:0}.quiz-page{break-after:page;page-break-after:always}.quiz-page:last-child{break-after:auto;page-break-after:auto}h1{font-size:14pt;margin:0 0 5mm}.questions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5mm 8mm}.question{break-inside:avoid;page-break-inside:avoid;font-size:9pt;line-height:1.25}.question p{margin:0 0 2mm}.question ol{margin:0;padding-left:5mm}.question li{margin:0 0 1mm}</style></head><body>${pages}</body></html>`);printWindow.document.close();}",
        "function renderQuestions(){quizDiv.innerHTML=''; const heading=document.createElement('h3'); heading.textContent='Forces questions'; quizDiv.appendChild(heading); selectedQuestionIndexes.forEach((questionIndex,slot)=>{const item=quizQuestions[questionIndex]; const article=document.createElement('article'); article.className='quiz-question'; const question=document.createElement('p'); question.textContent=`${slot+1}. ${item.question}`; article.appendChild(question); const options=document.createElement('ol'); options.className='quiz-options'; options.type='A'; item.options.forEach(option=>{const li=document.createElement('li');li.textContent=option;options.appendChild(li);}); article.appendChild(options); const answer=document.createElement('p'); answer.className='correct-answer'; answer.hidden=true; answer.textContent=`Correct answer: ${String.fromCharCode(65+item.correctAnswerIndex)}. ${item.options[item.correctAnswerIndex]}`; article.appendChild(answer); const controls=document.createElement('div'); controls.className='quiz-navigation'; const previous=document.createElement('button'); previous.type='button';previous.textContent='←';previous.setAttribute('aria-label',`Replace question ${slot+1} with the previous question`);previous.addEventListener('click',()=>swapQuestion(slot,-1)); const showAnswer=document.createElement('button'); showAnswer.type='button';showAnswer.textContent='Show answer';showAnswer.addEventListener('click',()=>{answer.hidden=false;showAnswer.remove();}); const next=document.createElement('button'); next.type='button';next.textContent='→';next.setAttribute('aria-label',`Replace question ${slot+1} with the next question`);next.addEventListener('click',()=>swapQuestion(slot,1)); controls.append(previous,showAnswer,next);article.appendChild(controls);quizDiv.appendChild(article);}); const printButton=document.createElement('button');printButton.type='button';printButton.textContent='Print quiz';printButton.addEventListener('click',printQuiz);quizDiv.appendChild(printButton);}",
        "async function generateQuiz(){if(sel.value!==QUIZ_UNIT)return; quizBtn.disabled=true; quizStatus.textContent='Loading questions...'; try{await loadQuizQuestions(); quizQuestions.sort(()=>Math.random()-.5); selectedQuestionIndexes=Array.from({length:Math.min(QUESTION_COUNT,quizQuestions.length)},(_,index)=>index); renderQuestions(); quizStatus.textContent='';}catch(error){quizStatus.textContent=error.message;}finally{if(sel.value===QUIZ_UNIT)quizBtn.disabled=false;}}",
