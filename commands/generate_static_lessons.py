...
- Fetches /sequences/{slug}/units, iterates each unit and tries:
    ...
    * GET /units/{unitSlug}?include=lessons
    ...
    if isinstance(j, list):
        for year_entry in j:
            ...
            if not units:
                ...
                if isinstance(year_entry, dict) and ("slug" in year_entry or "unitSlug" in year_entry or "id" in year_entry):
                    units = [year_entry]
            if not units:
                continue
            for u in units:
                if not isinstance(u, dict):
                    continue
                unit_slug = u.get("unitSlug") or u.get("slug") or u.get("id") or u.get("unit_slug")
