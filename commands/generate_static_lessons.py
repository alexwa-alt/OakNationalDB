...
def main():
    ...
    page = [
        ...
        "<p id=\"quiz-status\" class=\"quiz-status\">Select the Forces unit to use its five pre-generated quizzes.</p>",
        ...
        "function resetQuiz(){quizDiv.innerHTML=''; remainingQuizFiles=[]; quizFiles=null; quizBtn.disabled=true; quizStatus.textContent='Select the Forces unit to use its five pre-generated quizzes.';}",
        "function showPoints(unit){pointsDiv.innerHTML=''; resetQuiz(); if(!unit) return; const items=UNITS[unit]||[]; if(items.length===0){pointsDiv.textContent='No key learning points found.'; return;} const frag=document.createDocumentFragment(); items.forEach(p=>{const div=document.createElement('div');div.textContent=p;frag.appendChild(div)}); pointsDiv.appendChild(frag); copyBtn.textContent='Copy all'; if(unit===QUIZ_UNIT){quizBtn.disabled=false; quizStatus.textContent='Five pre-generated quizzes are available for this unit.';}}",
        ...
        "async function loadQuizFiles(){if(quizFiles!==null)return; const response=await fetch(QUIZ_INDEX_URL,{cache:'no-store'}); if(!response.ok)throw new Error('Quiz cache is not published yet.'); const data=await response.json(); if(!Array.isArray(data.quizzes)||!data.quizzes.every(name=>typeof name==='string'))throw new Error('Quiz cache index is invalid.'); quizFiles=data.quizzes; remainingQuizFiles=[...quizFiles].sort(()=>Math.random()-.5);}",
        "function renderQuiz(quiz){quizDiv.innerHTML=''; const heading=document.createElement('h3'); heading.textContent='Forces quiz'; quizDiv.appendChild(heading); quiz.questions.forEach((item,index)=>{const article=document.createElement('article'); article.className='quiz-question'; const question=document.createElement('p'); question.textContent=`${index+1}. ${item.question}`; article.appendChild(question); const options=document.createElement('ol'); options.className='quiz-options'; options.type='A'; item.options.forEach(option=>{const li=document.createElement('li');li.textContent=option;options.appendChild(li);}); article.appendChild(options); const answer=document.createElement('p'); answer.className='correct-answer'; answer.hidden=true; answer.textContent=`Correct answer: ${String.fromCharCode(65+item.correctAnswerIndex)}. ${item.options[item.correctAnswerIndex]}`; article.appendChild(answer); quizDiv.appendChild(article);}); const answers=document.createElement('button'); answers.type='button';answers.textContent='Show answers';answers.addEventListener('click',()=>{quizDiv.querySelectorAll('.correct-answer').forEach(answer=>answer.hidden=false);answers.remove();});quizDiv.appendChild(answers);}",
        "async function generateQuiz(){if(sel.value!==QUIZ_UNIT)return; quizBtn.disabled=true; quizStatus.textContent='Loading quiz...'; try{await loadQuizFiles(); const filename=remainingQuizFiles.pop(); if(!filename){quizStatus.textContent='All five cached quizzes have been shown.';return;} const response=await fetch(`quizzes/forces/${filename}`,{cache:'no-store'}); if(!response.ok)throw new Error('The selected quiz could not be loaded.'); const quiz=await response.json(); if(!Array.isArray(quiz.questions))throw new Error('The selected quiz is invalid.'); renderQuiz(quiz); quizStatus.textContent=`${remainingQuizFiles.length} cached quiz${remainingQuizFiles.length===1?'':'zes'} remaining.`;}catch(error){quizStatus.textContent=error.message;}finally{if(sel.value===QUIZ_UNIT&&remainingQuizFiles.length>0)quizBtn.disabled=false;}}",
